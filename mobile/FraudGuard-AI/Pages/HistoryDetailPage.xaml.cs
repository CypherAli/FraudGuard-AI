...
namespace FraudGuardAI.Pages
{
    ...
    public partial class HistoryDetailPage : ContentPage
    {
        ...
        private void PopulateUI()
        {
            ...
            // Keywords
            // Keywords — null-coalesce before Count to prevent NullReferenceException
            // if backend returns null for DetectedKeywords field
            var keywords = log.DetectedKeywords ?? new System.Collections.Generic.List<string>();
            if (keywords.Count > 0)
            {
        ...
        private async void OnShareClicked(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                // Fix 36: DetectedKeywords can be null when backend omits the field
                var kws = _callLog.DetectedKeywords;
                if (kws != null && kws.Count > 0)
                    shareText += $"\nPatterns: {string.Join(", ", kws)}\n";
